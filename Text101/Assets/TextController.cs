...
public class TextController : MonoBehaviour {
	...
	void sheets_0() {
		...
		if (Input.GetKeyDown(KeyCode.R)) {
			// setting the state
			myState = States.cell; // return to cell
		}
	}
	
	void lock_0() {
		text.text = "This is one of those button locks. You have no idea what the " +
					"the combination is. You wish you could somehow see where the dirty " +
	...
	void mirror() {
		...
		if (Input.GetKeyDown (KeyCode.R)) {
			...
			myState = States.cell_mirror; // take cell mirror
		}
	}
	...
	void lock_1() {
		text.text = "You carefully put the mirror through the bars, and turn it round " +
					...
					"Press C to go to corridor, or R to Return to your cell";
		if (Input.GetKeyDown (KeyCode.R)) {
			myState = States.cell_mirror;
		}
